...
namespace dso
{
PhotometricUndistorter::PhotometricUndistorter(std::string file,
                                               ...
                                               int w_, int h_)
{
...

    {
        ...
        if (GDepth < 256)
        {
            printf("PhotometricUndistorter: invalid format! got %d entries in "
                   "first line, expected at least 256!\n",
                   (int)Gvec.size());
            return;
        }
        ...
        for (int i = 0; i < GDepth - 1; i++)
        {
            if (G[i + 1] <= G[i])
            {
                printf("PhotometricUndistorter: G invalid! it has to be "
                       "strictly increasing, but it isnt!\n");
                return;
            }
        }
    ...
    if (vm16 != 0)
    {
        ...
        {
            printf("PhotometricUndistorter: Invalid vignette image size! got "
                   "%d x %d, expected %d x %d\n",
                   vm16->w, vm16->h, w, h);
            if (vm16 != 0)
                delete vm16;
            if (vm8 != 0)
                                          ...
                                          float factor)
{
    ...
    if (!valid || exposure_time <= 0 ||
        setting_photometricCalibration == 0)  // disable full photometric
                                              // calibration.
    {
        for (int i = 0; i < wh; i++)
        {
                                            ...
                                            std::string vignetteFilename)
{
    ...
    {
        f.close();
        printf(" ... not found. Cannot operate without calibration, shutting "
               "down.\n");
        f.close();
        return 0;
    }
    ...
    if (std::sscanf(l1.c_str(), "%f %f %f %f %f %f %f %f", &ic[0], &ic[1],
                    &ic[2], &ic[3], &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
                                       ...
                                       float factor) const
{
    ...
    {
        printf("Undistort::undistort: wrong image size (%d %d instead of %d "
               "%d) \n",
...
void Undistort::makeOptimalK_crop()
{
    ...
    // 2. while there are invalid pixels at the border: shrink square at the
    // side that has invalid pixels, if several to choose from, shrink the wider
    // dimension.
    ...
    while (oobLeft || oobRight || oobTop || oobBottom)
    {
        ...
        if (iteration > 500)
        {
            printf("FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING IS "
                   "SERIOUSLY WRONG. ABORTING \n");
            exit(1);
        }
    }
                             ...
                             std::string prefix)
{
    ...
    if (nPars == 5)  // fov model
    {
        ...
        {
            printf("Failed to read camera calibration (invalid "
                   "format?)\nCalibration file: %s\n",
                   configFileName);
            infile.close();
            return;
    ...
    {
        printf("called with invalid number of parameters.... forgot to "
               "implement me?\n");
        infile.close();
        return;
    }
    ...
    if (parsOrg[2] < 1 && parsOrg[3] < 1)
    {
        printf("\n\nFound fx=%f, fy=%f, cx=%f, cy=%f.\n I'm assuming this is "
               "the \"relative\" calibration file format,"
        ...
        // the 0.5 is because I'm assuming the calibration is given such that
        // the pixel at (0,0) contains the integral over intensity over
        // [0,0]-[1,1], whereas I assume the pixel (0,0) to contain a sample of
        // the intensity ot [0,0], which is best approximated by the integral
        // over
    ...
    if (std::sscanf(l4.c_str(), "%d %d", &w, &h) == 2)
    {
        if (benchmarkSetting_width != 0)
        {
            ...
            if (outputCalibration[0] == -3)
                outputCalibration[0] = -1;  // crop instead of none, since
                                            // probably resolution changed.
        }
                                         ...
                                         float* out_y, int n) const
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        float x_dist     = ix + ix * rad_dist_u + 2.0 * r1 * mxy_u +
                       r2 * (rho2_u + 2.0 * mx2_u);
        float y_dist = iy + iy * rad_dist_u + 2.0 * r2 * mxy_u +
                       r1 * (rho2_u + 2.0 * my2_u);
