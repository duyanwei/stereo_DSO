...
public:
                      ...
                      std::string gammaFile, std::string vignetteFile)
    {
        ...
        if (isZipped)
        {
            ...
            {
                const char* name =
                    zip_get_name(ziparchive, k, ZIP_FL_ENC_STRICT);
            ...
            std::sort(files.begin(), files.end());
#else
            printf("ERROR: cannot read .zip archive, as compile without "
                   "ziplib!\n");
            exit(1);
#endif
        }
...
private:
    MinimalImageB* getImageRaw_internal(int id, int unused)
    {
        ...
        else
        {
            ...
            long        readbytes = zip_fread(fle, databuffer,
            ...
            if (readbytes > (long)widthOrg * heightOrg * 6)
            {
                ...
                readbytes  = zip_fread(fle, databuffer,
