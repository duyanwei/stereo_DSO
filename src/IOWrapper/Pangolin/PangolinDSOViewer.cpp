...
void PangolinDSOViewer::run()
{
    ...
    pangolin::Var<bool> settings_showLiveResidual("ui.showResidual", false,
                                                  true);
    ...
    while (!pangolin::ShouldQuit() && running)
    {
        ...
        if (setting_render_display3D)
        {
            ...
            for (KeyFrameDisplay* fh : keyframes)
            {
                ...
                refreshed += (int)(fh->refreshPC(
                    refreshed < 10, this->settings_scaledVarTH,
                    this->settings_absVarTH, this->settings_pointCloudMode,
                    this->settings_minRelBS, this->settings_sparsity));
                fh->drawPC(1);
            }
        ...
        this->settings_showActiveConstraints =
            settings_showActiveConstraints.Get();
