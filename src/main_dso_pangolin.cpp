...
void settingsDefault(int preset)
{
    ...
    if (preset == 0 || preset == 1)
    {
        ...
        setting_kfGlobalWeight =
            1;  // original is 1.0. 0.3 is a balance between speed and accuracy.
...
void parseArgument(char* arg)
{
    ...
    if (1 == sscanf(arg, "save=%d", &option))
    {
        if (option == 1)
        {
            ...
            if (42 == system("rm -rf images_out"))
                printf("system call returned 42 - what are the odds?. This is "
    ...
    if (1 == sscanf(arg, "pointdensity=%d", &option))
    {
        ...
        setting_desiredImmatureDensity =
            int((double)setting_desiredPointDensity * 0.75);
        setting_minFrames        = 5;
        setting_maxFrames        = 7;
        setting_maxOptIterations = 6;
        setting_minOptIterations = 1;
...
int main(int argc, char** argv)
{
    ...
    // ImageFolderReader* reader = new ImageFolderReader(source+"/image_0",
    // calib, gammaCalib, vignette); ImageFolderReader* reader_right = new
    // ImageFolderReader(source+"/image_1", calib, gammaCalib, vignette);
    // reader->setGlobalCalibration();
    // reader_right->setGlobalCalibration();
    ImageFolderReader* reader =
        new ImageFolderReader(source0, calib0, gammaCalib, vignette);
    ImageFolderReader* reader_right =
        new ImageFolderReader(source1, calib1, gammaCalib, vignette);
    reader->setGlobalCalibration();
    // reader_right->setGlobalCalibration();
    int w_out, h_out;
    ...
    std::thread runthread([&]() {
             ...
             i += linc)
        {
            ...
            {
                double tsThis =
                    reader->getTimestamp(idsToPlay[idsToPlay.size() - 1]);
                double tsPrev =
                    reader->getTimestamp(idsToPlay[idsToPlay.size() - 2]);
                timesToPlayAt.push_back(timesToPlayAt.back() +
                                        fabs(tsThis - tsPrev) / playbackSpeed);
            }
        }
        for (int i = lstart; i >= 0 && i < reader_right->getNumImages() &&
                             linc * i < linc * lend;
             i += linc)
        {
        ...
        for (int ii = 0; ii < (int)idsToPlay.size(); ii++)
        {
            if (!fullSystem->initialized)  // if not initialized: reset start
                                           // time.
            {
                gettimeofday(&tv_start, NULL);
                started            = clock();
            ...
            //     {
            //         if (pic_time_stamp_r[i] >= time_l || fabs(
            //         pic_time_stamp_r[i] - time_l) < 0.01)
            //         {
            //             index = i;
            //             break;
            //         }
            ...
            // std::cout << std::setprecision(20) << time_l << ", " <<
            // pic_time_stamp_r[index] << "\n";
            ...
            if (playbackSpeed != 0)
            {
                ...
                if (sSinceStart < timesToPlayAt[ii])
                    usleep(
                        (int)((timesToPlayAt[ii] - sSinceStart) * 1000 * 1000));
