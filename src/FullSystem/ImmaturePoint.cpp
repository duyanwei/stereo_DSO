  ...
  , lastTraceStatus(IPS_UNINITIALIZED)
{
    ...
    for (int idx = 0; idx < patternNum; idx++)
    {
        ...
        Vec3f ptc =
            getInterpolatedElement33BiLin(host->dI, u + dx, v + dy, wG[0]);
...
ImmaturePoint::ImmaturePoint(float u_, float v_, FrameHessian* host_,
                             CalibHessian* HCalib)
  : u(u_)
  , v(v_)
  , host(host_)
                                           ...
                                           bool          debugPrint)
{
    ...
    if (debugPrint)
        printf("trace pt (%.1f %.1f) from frame %d to %d. Range %f -> %f. t %f "
               "%f %f!\n",
               u, v, host->shell->id, frame->shell->id, idepth_min, idepth_max,
               hostToFrame_Kt[0], hostToFrame_Kt[1], hostToFrame_Kt[2]);

    //	const float stepsize = 1.0;				// stepsize for initial discrete
    // search. 	const int GNIterations = 3;				// max # GN iterations
    //	const float GNThreshold = 0.1;				// GN stop after this
    // stepsize. 	const float extraSlackOnTH = 1.2;			// for
    // energy-based outlier check, be slightly more relaxed by this factor.
    // const float slackInterval = 0.8;			// if pixel-interval is smaller
    // than this, leave it be. 	const float minImprovementFactor = 2; // if
    // pixel-interval is smaller than this, leave it be.
    ...
    for (int idx = 0; idx < patternNum; idx++)
        rotatetPattern[idx] =
            Rplane * Vec2f(patternP[idx][0], patternP[idx][1]);
    ...
    for (int i = 0; i < numSteps; i++)
    {
        ...
        for (int idx = 0; idx < patternNum; idx++)
        {
            ...
            float residual =
                hitColor - (float)(hostToFrame_affine[0] * color[idx] +
    ...
    for (int it = 0; it < setting_trace_GNIterations; it++)
    {
        ...
        for (int idx = 0; idx < patternNum; idx++)
        {
            ...
            energy += weights[idx] * weights[idx] * hw * residual * residual *
                      (2 - hw);
        }
        ...
        if (energy > bestEnergy)
        {
            ...
            if (debugPrint)
                printf("GN BACK %d: E %f, H %f, b %f. id-step %f. UV %f %f -> "
                       "%f %f.\n",
        ...
        else
        {
            ...
            if (debugPrint)
                printf("GN step %d: E %f, H %f, b %f. id-step %f. UV %f %f -> "
                       "%f %f.\n",
...
// do static stereo match. if mode_right = true, it matches from left to right.
// otherwise do it from right to left.
                                               ...
                                               bool          mode_right)
{
    ...
    if (errorInPixel * setting_trace_minImprovementFactor > dist &&
        std::isfinite(idepth_max_stereo))
    {
        return lastTraceStatus = ImmaturePointStatus ::IPS_BADCONDITION;
    }
    ...
    if (dx * dx > dy * dy)
    {
        idepth_min_stereo = (pr[2] * (bestU - errorInPixel * dx) - pr[0]) /
                            (Kt[0] - Kt[2] * (bestU - errorInPixel * dx));
        idepth_max_stereo = (pr[2] * (bestU + errorInPixel * dx) - pr[0]) /
                            (Kt[0] - Kt[2] * (bestU + errorInPixel * dx));
    }
    else
    {
        idepth_min_stereo = (pr[2] * (bestV - errorInPixel * dy) - pr[1]) /
                            (Kt[1] - Kt[2] * (bestV - errorInPixel * dy));
        idepth_max_stereo = (pr[2] * (bestV + errorInPixel * dy) - pr[1]) /
                            (Kt[1] - Kt[2] * (bestV + errorInPixel * dy));
    }
    ...
    if (!std::isfinite(idepth_min_stereo) ||
        !std::isfinite(idepth_max_stereo) || (idepth_max_stereo < 0))
    {
        lastTracePixelInterval = 0;
        lastTraceUV            = Vec2f(-1, -1);
        return lastTraceStatus = ImmaturePointStatus::IPS_OUTLIER;
                                  ...
                                  float                           idepth)
{
    ...
    {
        float Ku, Kv;
        if (!projectPoint(this->u + patternP[idx][0],
                          this->v + patternP[idx][1], idepth, PRE_KRKiTll,
                          PRE_KtTll, Ku, Kv))
        {
            return 1e10;
        }
