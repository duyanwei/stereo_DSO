...
void FullSystem::flagFramesForMarginalization(FrameHessian* newFH)
{
    ...
    for (int i = 0; i < (int)frameHessians.size(); i++)
    {
        ...
        {
            //			printf("MARGINALIZE frame %d, as only %'d/%'d points
            // remaining (%'d %'d %'d %'d). VisInLast %'d / %'d. traces %d,
            // activated %d!\n", 					fh->frameID, in, in+out,
            //					(int)fh->pointHessians.size(),
            //(int)fh->immaturePoints.size(),
            //					(int)fh->pointHessiansMarginalized.size(),
            //(int)fh->pointHessiansOut.size(), 					visInLast,
            // outInLast, fh->statistics_tracesCreatedForThisFrame,
        ...
        {
            //			printf("May Keep frame %d, as %'d/%'d points remaining
            //(%'d
            //%'d %'d %'d). VisInLast %'d / %'d. traces %d, activated %d!\n",
            //					fh->frameID, in, in+out,
            //					(int)fh->pointHessians.size(),
            //(int)fh->immaturePoints.size(),
            //					(int)fh->pointHessiansMarginalized.size(),
    ...
    if ((int)frameHessians.size() - flagged >= setting_maxFrames)
    {
        ...
        for (FrameHessian* fh : frameHessians)
        {
            ...
            {
                if (ffh.target->frameID >
                        latest->frameID - setting_minFrameAge + 1 ||
                    ffh.target == ffh.host)
                    continue;
