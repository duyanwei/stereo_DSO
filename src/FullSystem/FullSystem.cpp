...
FullSystem::FullSystem()
{
    ...
    if (setting_logStuff)
    {
        ...
        eigenAllLog->open("logs/eigenAllLog.txt",
                          std::ios::trunc | std::ios::out);
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
    ...
    else
    {
        ...
        {  // lock on global pose consistency!
            boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
            slast_2_sprelast =
                sprelast->camToWorld.inverse() * slast->camToWorld;
            lastF_2_slast =
                slast->camToWorld.inverse() * lastF->shell->camToWorld;
        ...
        lastF_2_fh_tries.push_back(fh_2_slast.inverse() * fh_2_slast.inverse() *
                                   lastF_2_slast);  // assume double motion
                                                    // (frame skipped)
        ...
        // end, if they don't work they will only be tried on the coarsest
        // level, which is super fast anyway. also, if tracking rails here we
        // loose, so we really, really want to avoid that.
    ...
    // the options. I'll keep track of the so-far best achieved residual for
    // each level in achievedRes. If on a coarse level, tracking is WORSE than
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++)
    {
        ...
        // 		LOG(INFO)<<"lastF_2_fh_this:
        // "<<lastF_2_fh_this.translation().transpose();
        // 		LOG(INFO)<<"aff_g2l_this: "<<aff_g2l_this.vec().transpose();
        bool trackingIsGood = coarseTracker->trackNewestCoarse(
            fh, lastF_2_fh_this, aff_g2l_this, pyrLevelsUsed - 1,
            achievedRes);  // in each level has to be at least as good as the
                           // last try.
        tryIterations++;
        // 		LOG(INFO)<<"coarseTracker->lastResiduals[0]:
        ...
        if (false && i != 0)
        {
            printf("RE-TRACK ATTEMPT %d with initOption %d and start-lvl %d "
                   ...
                   achievedRes[0], achievedRes[1], achievedRes[2],
                   achievedRes[3], achievedRes[4],
                   coarseTracker->lastResiduals[0],
        ...
        if (haveOneGood)
        {
            for (int i = 0; i < 5; i++)
            {
                if (!std::isfinite((float)achievedRes[i]) ||
                    achievedRes[i] >
                        coarseTracker->lastResiduals[i])  // take over if
                                                          // achievedRes is
                                                          // either bigger or
                                                          // NAN.
    ...
    if (!haveOneGood)
    {
        printf("BIG ERROR! tracking failed entirely. Take predictred pose and "
               "hope we may somehow recover.\n");
        flowVecs   = Vec3(0, 0, 0);
    ...
    for (FrameHessian* host : frameHessians)  // go through all active frames
    {
        ...
        for (ImmaturePoint* ph : host->immaturePoints)
        {
            ...
            if (phTrackStatus == ImmaturePointStatus::IPS_GOOD)
            {
                ...
                Vec3f ptpMin =
                    KRKi * (Vec3f(ph->u, ph->v, 1) / ph->idepth_min) + Kt;
                float idepth_min_project = 1.0f / ptpMin[2];
                Vec3f ptpMax =
                    KRKi * (Vec3f(ph->u, ph->v, 1) / ph->idepth_max) + Kt;
                ...
                ImmaturePointStatus phNonKeyStereoStatus =
                    phNonKey->traceStereo(fh_right, &Hcalib, 1);
                if (phNonKeyStereoStatus == ImmaturePointStatus::IPS_GOOD)
                {
                    ImmaturePoint* phNonKeyRight = new ImmaturePoint(
                        phNonKey->lastTraceUV(0), phNonKey->lastTraceUV(1),
                        fh_right, &Hcalib);
    ...
    for (FrameHessian* host : frameHessians)  // go through all active frames
    {
        ...
        for (ImmaturePoint* ph : host->immaturePoints)
        {
            ImmaturePointStatus phTrackStatus =
                ph->traceOn(fh, KRKi, Kt, aff, &Hcalib, false);
            // 				if(ph->u==145&&ph->v==21){
            // 				    LOG(INFO)<<"trace depthmax: "<<ph->idepth_max<<"
            // min:
            // "<<ph->idepth_min;
            // 				}
            if (ph->lastTraceStatus == ImmaturePointStatus::IPS_GOOD)
...
void FullSystem::activatePointsMT()
{
    ...
    for (FrameHessian* host : frameHessians)  // go through all active frames
    {
        ...
        for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1)
        {
            ...
            // delete points that have never been traced successfully, or that
            // are outlier on the last trace.
            ...
            {
                float dist =
                    coarseDistanceMap->fwdWarpedIDDistFinal[u + wG[1] * v] +
    ...
    if (multiThreading)
        treadReduce.reduce(boost::bind(&FullSystem::activatePointsMT_Reductor,
                                       this, &optimized, &toOptimize, _1, _2,
                                       _3, _4),
        ...
        for (unsigned int i = 0; i < host->pointHessians.size(); i++)
        {
                     ...
                     host->flaggedForMarginalization)
            {
                ...
                else
                {
                    ...
                    // printf("drop point in frame %d (%d goodRes, %d
                                ...
                                ImageAndExposure* image_right, int id)
{
    ...
    {
        // use initializer!
        if (coarseInitializer->frameID < 0)  // first frame set. fh is kept by
                                             // coarseInitializer.
        {
    ...
    else  // do front-end operation.
    {
        ...
        else
        {
            ...
            // 			LOG(INFO)<<"tres: "<<tres.transpose()<<" refToFh[0]:
            // "<<refToFh[0];
        }

        // 		LOG(INFO)<<"needToMakeKF: "<<(int)needToMakeKF;
        // 		LOG(INFO)<<"coarseTracker->firstCoarseRMSE:
        // "<<coarseTracker->firstCoarseRMSE<<" tres[0]:"<<tres[0];
        ...
        // "<<allFrameHistory[allFrameHistory.size()-1]->aff_g2l.vec().transpose();
        // 		LOG(INFO)<<"fh->shell->camToTrackingRef:
        // "<<fh->shell->camToTrackingRef.translation().transpose();
...
void FullSystem::mappingLoop()
{
    ...
    while (runMapping)
    {
        ...
        if (unmappedTrackedFrames.size() > 0)  // if there are other frames to
                                               // tracke, do that first.
        {
            lock.unlock();
            makeNonKeyFrame(fh, fh_right);
            lock.lock();
...
{
    // needs to be set by mapping thread. no lock required since we are in
    // mapping thread.
    {
        boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
        assert(fh->shell->trackingRef != 0);
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
    ...
    for (int i = 0; i < coarseInitializer->numPoints[0]; i++)
    {
        ...
        PointFrameResidual* r =
            new PointFrameResidual(ph, ph->host, ph->host->frame_right);
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
    ...
    int numPointsTotal = pixelSelector->makeMaps(
        newFrame, selectionMap, setting_desiredImmatureDensity);
...
void FullSystem::printLogLine()
{
    ...
    if (!setting_debugout_runquiet)
        printf("LOG %d: %.3f fine. Res: %d A, %d L, %d M; (%'d / %'d) "
               "forceDrop. a=%f, b=%f. Window %d (%d)\n",
...
void FullSystem::printEigenValLine()
{
    ...
    if (eigenAllLog != 0)
    {
        ...
        (*eigenAllLog) << allKeyFramesHistory.back()->id << " "
                       << ea.transpose() << "\n";
        eigenAllLog->flush();
    }
    if (eigenALog != 0)
    ...
    if (DiagonalLog != 0)
    {
        ...
        (*DiagonalLog) << allKeyFramesHistory.back()->id << " "
                       << ea.transpose() << "\n";
        DiagonalLog->flush();
    }
    ...
    if (variancesLog != 0)
    {
        ...
        (*variancesLog) << allKeyFramesHistory.back()->id << " "
                        << ea.transpose() << "\n";
        variancesLog->flush();
    }
