...
inline Vec2 affFromTo(const Vec2& from,
                      const Vec2& to)  // contains affine parameters as
                                       // XtoWorld.
{
...
struct FrameHessian
{
    ...
    Eigen::Vector3f* dI;  // trace, fine tracking. Used for direction select
                          // (not for gradient histograms etc.)
    Eigen::Vector3f* dIp[PYR_LEVELS];   // coarse tracking / coarse initializer.
                                        // NAN in [0] only.
    float* absSquaredGrad[PYR_LEVELS];  // only used for pixel select
                                        // (histograms etc.). no NAN.
    ...
    std::vector<PointHessian*> pointHessiansMarginalized;  // contains all
                                                           // MARGINALIZED
                                                           // points (= fully
                                                           // marginalized,
                                                           // usually because
                                                           // point went OOB.)
    std::vector<PointHessian*> pointHessiansOut;  // contains all OUTLIER points
                                                  // (= discarded.).
    std::vector<ImmaturePoint*> immaturePoints;   // contains all OUTLIER points
                                                  // (= discarded.).
    ...
    inline void setStateScaled(const Vec10& state_scaled)
    {
        ...
        state.segment<3>(0) =
            SCALE_XI_TRANS_INVERSE * state_scaled.segment<3>(0);
...
struct CalibHessian
{
    ...
    inline void setValue(const VecC& value)
    {
        ...
        this->value_scaledi[2] =
            -this->value_scaledf[2] / this->value_scaledf[0];
        this->value_scaledi[3] =
            -this->value_scaledf[3] / this->value_scaledf[1];
...
struct PointHessian
{
    ...
    std::vector<PointFrameResidual*> residuals;  // only contains good residuals
                                                 // (not OOB and not OUTLIER).
                                                 // Arbitrary order.
    std::pair<PointFrameResidual*, ResState>
        lastResiduals[2];  // contains information about residuals to the last
                           // two (!) frames. ([0] = latest, [1] = the one
                           // before).
