...
void CoarseTracker::makeK(CalibHessian* HCalib)
{
    ...
    {
        K[level] << fx[level], 0.0, cx[level], 0.0, fy[level], cy[level], 0.0,
            0.0, 1.0;
...
void CoarseTracker::makeCoarseDepthForFirstFrame(FrameHessian* fh)
{
    ...
    for (int lvl = 0; lvl < 2; lvl++)
    {
        ...
        for (int it = 0; it < numIts; it++)
        {
            ...
            memcpy(weightSumsl_bak, weightSumsl,
                   w[lvl] * h[lvl] * sizeof(float));
            float* idepthl = idepth[lvl];  // dont need to make a temp copy of
                                           // depth, since I only
    ...
    for (int lvl = 2; lvl < pyrLevelsUsed; lvl++)
    {
        ...
        float* idepthl = idepth[lvl];  // dotnt need to make a temp copy of
                                       // depth, since I only
                                      ...
                                      CalibHessian               Hcalib)
{
    ...
    for (FrameHessian* fh : frameHessians)
    {
        for (PointHessian* ph : fh->pointHessians)
        {
            if (ph->lastResiduals[0].first != 0 &&
                ph->lastResiduals[0].second ==
                    ResState::IN)  // contains information about residuals to
                                   // the last two (!) frames. ([0] = latest,
                                   // [1] = the one before).
            {
                PointFrameResidual* r = ph->lastResiduals[0].first;
                assert(r->efResidual->isActive() && r->target == lastRef);
                ...
                ImmaturePoint* pt_track =
                    new ImmaturePoint((float)u, (float)v, fh_target, &Hcalib);
                ...
                ImmaturePointStatus pt_track_right =
                    pt_track->traceStereo(fh_right, &Hcalib, 1);

                float new_idepth = 0;

                if (pt_track_right == ImmaturePointStatus::IPS_GOOD)
                {
                    ImmaturePoint* pt_track_back = new ImmaturePoint(
                        pt_track->lastTraceUV(0), pt_track->lastTraceUV(1),
                        fh_right, &Hcalib);
                    pt_track_back->u_stereo = pt_track_back->u;
                    pt_track_back->v_stereo = pt_track_back->v;

                    pt_track_back->idepth_min_stereo =
                        r->centerProjectedTo[2] * 0.1f;
                    pt_track_back->idepth_max_stereo =
                        r->centerProjectedTo[2] * 1.9f;

                    ImmaturePointStatus pt_track_left =
                        pt_track_back->traceStereo(fh_target, &Hcalib, 0);

                    float depth = 1.0f / pt_track->idepth_stereo;
                    float u_delta =
                        abs(pt_track->u - pt_track_back->lastTraceUV(0));
    ...
    for (int lvl = 0; lvl < 2; lvl++)
    {
        ...
        for (int it = 0; it < numIts; it++)
        {
            ...
            memcpy(weightSumsl_bak, weightSumsl,
                   w[lvl] * h[lvl] * sizeof(float));
            float* idepthl =
                idepth[lvl];  // dotnt need to make a temp copy of depth, since
                              // I only read values with weightSumsl>0, and
                              // write ones with weightSumsl<=0.
            for (int i = w[lvl]; i < wh; i++)
            {
                if (weightSumsl_bak[i] <= 0)
                {
    ...
    for (int lvl = 2; lvl < pyrLevelsUsed; lvl++)
    {
        ...
        memcpy(weightSumsl_bak, weightSumsl, w[lvl] * h[lvl] * sizeof(float));
        float* idepthl =
            idepth[lvl];  // dotnt need to make a temp copy of depth, since I
                          // only read values with weightSumsl>0, and write ones
                          // with weightSumsl<=0.
        for (int i = w[lvl]; i < wh; i++)
        {
            if (weightSumsl_bak[i] <= 0)
...
void CoarseTracker::setCTRefForFirstFrame(
...
void CoarseTracker::setCoarseTrackingRef(
    std::vector<FrameHessian*> frameHessians, FrameHessian* fh_right,
    CalibHessian Hcalib)
{
    assert(frameHessians.size() > 0);
    lastRef = frameHessians.back();
    ...
    for (int lvl = coarsestLvl; lvl >= 0; lvl--)
    {
        ...
        if (debugPrint)
        {
            ...
            printf("lvl%d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> %d) (|inc| "
                   "= %f)! \t",
                   lvl, -1, lambda, 1.0f, "INITIA", 0.0f, resOld[0] / resOld[1],
                   0, (int)resOld[1], 0.0f);
        ...
        for (int iteration = 0; iteration < maxIterations[lvl]; iteration++)
        {
            ...
            {
                inc.head<6>() =
                    Hl.topLeftCorner<6, 6>().ldlt().solve(-b.head<6>());
                inc.tail<2>().setZero();
            }
            if (!(setting_affineOptModeA < 0) &&
            ...
            {
                inc.head<7>() =
                    Hl.topLeftCorner<7, 7>().ldlt().solve(-b.head<7>());
                inc.tail<1>().setZero();
            }
            if (setting_affineOptModeA < 0 &&
                !(setting_affineOptModeB < 0))  // fix a
            {
                ...
                bStitch[6]      = bStitch[7];
                Vec7 incStitch  = HlStitch.topLeftCorner<7, 7>().ldlt().solve(
                    -bStitch.head<7>());
                inc.setZero();
