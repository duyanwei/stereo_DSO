...
PointHessian* FullSystem::optimizeImmaturePoint(
    ImmaturePoint* point, int minObs, ImmaturePointTemporaryResidual* residuals)
{
    int nres = 0;
    ...
    {
        if (print)
            printf("OptPoint: Not well-constrained (%d res, H=%.1f). E=%f. "
                   "SKIP!\n",
    ...
    if (print)
        printf("Activate point. %d residuals. H=%f. Initial Energy: %f. "
               "Initial Id=%f\n",
         ...
         iteration++)
    {
        ...
        if (print)
            printf("%s %d (L %.2f) %s: %f -> %f (idepth %f)!\n",
                   ...
                   iteration, log10(lambda), "", lastEnergy, newEnergy,
                   newIdepth);
    ...
    if (!std::isfinite(currentIdepth))
    {
        printf("MAJOR ERROR! point idepth is nan after initialization (%f).\n",
               currentIdepth);
        return (PointHessian*)((long)(-1));  // yeah I'm like 99% sure this is
                                             // OK on 32bit systems.
    }
    ...
    PointFrameResidual* r =
        new PointFrameResidual(p, p->host, p->host->frame_right);
