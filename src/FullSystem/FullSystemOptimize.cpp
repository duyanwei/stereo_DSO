                ...
                if (r->isNew && r->stereoResidualFlag == false)
                {
                    ...
                    Vec3f ptp =
                        ...
                        r->host->targetPrecalc[r->target->idx].PRE_KtTll *
                            p->idepth_scaled;  // projected point with real
                                               // depth.
...
float FullSystem::optimize(int mnumOptIts)
{
    ...
    for (int iteration = 0; iteration < mnumOptIts; iteration++)
    {
            ...
            (setting_solverMode & SOLVER_STEPMOMENTUM))
        {
            ...
            stepsize =
                sqrtf(sqrtf(newStepsize * stepsize * stepsize * stepsize));
            if (stepsize > 2)
                stepsize = 2;
            if (stepsize < 0.25)
                stepsize = 0.25;
    ...
    newStateZero.segment<2>(6) =
        frameHessians.back()->get_state().segment<2>(6);
