                                   ...
                                   std::vector<IOWrap::Output3DWrapper*>& wraps)
{
    ...
    for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--)
    {
        ...
        if (printDebug)
        {
            printf("lvl %d, it %d (l=%f) %s: %.3f+%.5f -> %.3f+%.5f "
                   "(%.3f->%.3f) (|inc| = %f)! \t",
                   lvl, 0, lambda, "INITIA",
        ...
        while (true)
        {
            ...
            if (printDebug)
            {
                printf("lvl %d, it %d (l=%f) %s: %.5f + %.5f + %.5f -> %.5f + "
                       "%.5f + %.5f (%.2f->%.2f) (|inc| = %f)! \t",
                                      ...
                                      AffLight refToNew_aff, bool plot)
{
    ...
    for (int i = 0; i < npts; i++)
    {
        ...
        {
            point->energy_new[1] =
                (point->idepth_new - 1) * (point->idepth_new - 1);
...
Vec3f CoarseInitializer::calcEC(int lvl)
{
    ...
    // printf("ER: %f %f %f!\n", couplingWeight*E.A1m[0],
    // couplingWeight*E.A1m[1], (float)E.num.numIn1m);
...
void CoarseInitializer::optReg(int lvl)
{
    ...
    for (int i = 0; i < npts; i++)
    {
        ...
        if (nnn > 2)
        {
            ...
            point->iR =
                (1 - regWeight) * point->idepth + regWeight * idnn[nnn / 2];
        }
    }
}
...
void CoarseInitializer::makeGradients(Eigen::Vector3f** data)
{
    for (int lvl = 1; lvl < pyrLevelsUsed; lvl++)
    {
        ...
        {
            dINew_l[idx][1] =
                0.5f * (dINew_l[idx + 1][0] - dINew_l[idx - 1][0]);
            dINew_l[idx][2] =
                0.5f * (dINew_l[idx + wl][0] - dINew_l[idx - wl][0]);
        }
    }
}
                                 ...
                                 FrameHessian* newFrameHessian)
{
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++)
    {
        ...
        int npts;
        if (lvl == 0)
            npts = sel.makeMaps(firstFrame, statusMap,
                                densities[lvl] * w[0] * h[0], 1, false, 2);
        else
            npts = makePixelStatus(firstFrame->dIp[lvl], statusMapB, w[lvl],
                                   h[lvl], densities[lvl] * w[0] * h[0]);
            ...
            for (int x = patternPadding + 1; x < wl - patternPadding - 2; x++)
            {
                    ...
                    (lvl == 0 && statusMap[x + y * wl] != 0))
                {
                    ...
                    for (int idx = 0; idx < patternNum; idx++)
                    {
                        ...
                        int   dy = patternP[idx][1];
                        float absgrad =
                            cpt[dx + dy * w[lvl]].tail<2>().squaredNorm();
                        sumGrad2 += absgrad;
                    }
            ...
            for (int x = patternPadding + 1; x < wl - patternPadding - 2; x++)
            {
                if (lvl == 0 && statusMap[x + y * wl] != 0)
                {
                    ImmaturePoint* pt = new ImmaturePoint(
                        x, y, firstFrame, statusMap[x + y * wl], HCalib);
                    pt->u_stereo          = pt->u;
                    pt->v_stereo          = pt->v;
                    pt->idepth_min_stereo = 0;
                    pt->idepth_max_stereo = NAN;
                    ImmaturePointStatus stat =
                        pt->traceStereo(firstFrame_right, HCalib, true);
                    ...
                    // pl[nl].idepth); 					pl[nl].iR = 0.01;
                    // pl[nl].isGood=true; pl[nl].energy.setZero();
                    // 					pl[nl].lastHessian=0;
                    // 					pl[nl].lastHessian_new=0;
                    // 					pl[nl].my_type= (lvl!=0) ? 1 :
                    // statusMap[x+y*wl]; 					idepth[0][x+wl*y] =
                    // 0.01;
                    //
                    // 					Eigen::Vector3f* cpt = firstFrame->dIp[lvl] +
                    // x
                    // + y*w[lvl]; 					float sumGrad2=0;
                    // for(int idx=0;idx<patternNum;idx++)
                    // 					{
                    // 					    int dx = patternP[idx][0];
                    // 					    int dy = patternP[idx][1];
                    // 					    float absgrad = cpt[dx +
                    // dy*w[lvl]].tail<2>().squaredNorm(); 					    sumGrad2
                    // += absgrad;
                    // 					}
                    //
                    // 					pl[nl].outlierTH =
                    // patternNum*setting_outlierTH;
                    //
                    // 					nl++;
                    // 					assert(nl <= npts);
                    // 				}
                ...
                // 				pl[nl].lastHessian_new=0;
                // 				pl[nl].my_type= (lvl!=0) ? 1 :
                // statusMap[x+y*wl]; 				int bidx = 2*x   + 2*y*wlm1;
                // idepth_l[x + y*wl] = idepth_lm[bidx] +
                // idepth_lm[bidx+1] +
                // idepth_lm[bidx+wlm1]
                // + idepth_lm[bidx+wlm1+1];
                //
                // 				Eigen::Vector3f* cpt = firstFrame->dIp[lvl] + x
                // + y*w[lvl]; 				float sumGrad2=0; for(int
                // idx=0;idx<patternNum;idx++)
                // 				{
                // 					int dx = patternP[idx][0];
                // 					int dy = patternP[idx][1];
                // 					float absgrad = cpt[dx +
...
void CoarseInitializer::makeNN()
{
    ...
    typedef nanoflann::KDTreeSingleIndexAdaptor<
        nanoflann::L2_Simple_Adaptor<float, FLANNPointcloud>, FLANNPointcloud,
        2>
        KDTree;
