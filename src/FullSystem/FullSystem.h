...
public:
    ...
    void addActiveFrame(ImageAndExposure* image, ImageAndExposure* image_right,
                        int id);
...
private:
    ...
    double linAllPointSinle(PointHessian* point, float outlierTHSlack,
                            bool plot);
    ...
    std::vector<FrameHessian*> frameHessians;  // ONLY changed in
                                               // marginalizeFrame and addFrame.
    std::vector<FrameHessian*> frameHessians_right;  // ONLY changed in
                                                     // marginalizeFrame and
                                                     // addFrame.
    ...
    boost::mutex coarseTrackerSwapMutex;  // if tracker sees that there is a new
                                          // reference, tracker locks
                                          // [coarseTrackerSwapMutex] and swaps
                                          // the two.
    CoarseTracker* coarseTracker_forNewKF;  // set as as reference. protected by
                                            // [coarseTrackerSwapMutex].
    CoarseTracker* coarseTracker;  // always used to track new frames. protected
                                   // by [trackMutex].
    float minIdJetVisTracker, maxIdJetVisTracker;
    float minIdJetVisDebug, maxIdJetVisDebug;

    // mutex for camToWorl's in shells (these are always in a good
    // configuration).
    ...
    void deliverTrackedFrame(FrameHessian* fh, FrameHessian* fh_right,
                             bool needKF);
    void mappingLoop();
    ...
    int needNewKFAfter;  // Otherwise, a new KF is *needed that has ID bigger
                         // than [needNewKFAfter]*.
