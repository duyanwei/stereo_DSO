...
void EnergyFunctional::setAdjointsF(CalibHessian* Hcalib)
{
    ...
    for (int h = 0; h < nFrames; h++)
        for (int t = 0; t < nFrames; t++)
        {
            ...
            SE3 hostToTarget = target->get_worldToCam_evalPT() *
            ...
            Vec2f affLL = AffLight::fromToVecExposure(
                              host->ab_exposure, target->ab_exposure,
    ...
    for (int h = 0; h < nFrames; h++)
        for (int t = 0; t < nFrames; t++)
        {
            ...
            adTargetF[h + t * nFrames] =
                adTarget[h + t * nFrames].cast<float>();
        }
...
void EnergyFunctional::setDeltaF(CalibHessian* HCalib)
{
    ...
    for (EFFrame* f : frames)
    {
        ...
        f->delta_prior =
            (f->data->get_state() - f->data->getPriorZero()).head<8>();
...
void EnergyFunctional::accumulateAF_MT(MatXX& H, VecX& b, bool MT)
{
    if (MT)
    {
        red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero,
                                accSSE_top_A, nFrames, _1, _2, _3, _4),
                    0, 0, 0);
...
void EnergyFunctional::accumulateLF_MT(MatXX& H, VecX& b, bool MT)
{
    ...
    {
        red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero,
                                accSSE_top_L, nFrames, _1, _2, _3, _4),
                    0, 0, 0);
    ...
    for (int i = min; i < max; i++)
    {
        ...
        for (EFResidual* r : p->residualsAll)
        {
            ...
            for (int i = 0; i + 3 < patternNum; i += 4)
            {
                ...
                __m128 Jdelta = _mm_mul_ps(
                    _mm_load_ps(((float*)(rJ->JIdx)) + i), Jp_delta_x);
                Jdelta = _mm_add_ps(
                    Jdelta,
                ...
                Jdelta = _mm_add_ps(
                    Jdelta,
                    _mm_mul_ps(_mm_load_ps(((float*)(rJ->JabF + 1)) + i),
                               delta_b));
        ...
        for (int i = 0; i < (int)f->points.size(); i++)
        {
            ...
            if (p->stateFlag == EFPointStatus::PS_MARGINALIZE)
            {
                ...
                for (EFResidual* r : p->residualsAll)
                    if (r->isActive())
                        if (r->data->stereoResidualFlag == false)
                            connectivityMap[(((uint64_t)r->host->frameID)
                                             << 32) +
                                            ((uint64_t)r->target->frameID)]
                                           [1]++;
                //                         connectivityMap[(((uint64_t)r->host->frameID)
                //                         << 32) +
...
void EnergyFunctional::orthogonalize(VecX* b, MatXX* H)
{
    ...
    ns.insert(ns.end(), lastNullspaces_scale.begin(),
    ...
    MatXX NNpiTS =
        0.5 * (NNpiT + NNpiT.transpose());  // = N * (N' * N)^-1 * N'.
                                    ...
                                    CalibHessian* HCalib)
{
    ...
    if (setting_solverMode & SOLVER_SVD)
    {
        ...
        MatXX HFinalScaled =
            SVecI.asDiagonal() * HFinal_top * SVecI.asDiagonal();
        VecX                    bFinalScaled = SVecI.asDiagonal() * bFinal_top;
        Eigen::JacobiSVD<MatXX> svd(HFinalScaled,
    ...
    else
    {
        ...
        MatXX HFinalScaled =
            SVecI.asDiagonal() * HFinal_top * SVecI.asDiagonal();
        x = SVecI.asDiagonal() *
            HFinalScaled.ldlt().solve(SVecI.asDiagonal() *
                                      bFinal_top);  //  SVec.asDiagonal() *
                                                    //  svd.matrixV() * Ub;
    }
