    ...
    Vec10* stats, int tid)
{
    ...
    if (min == 0)
    {
        for (int tid2 = 0; tid2 < toAggregate; tid2++)
        {
            accHcc[tid2].finish();
            accbc[tid2].finish();
            H[tid].topLeftCorner<CPARS, CPARS>() +=
                accHcc[tid2].A1m.cast<double>();
    ...
    for (int i = 0; i < nf; i++)
        for (int j = 0; j < nf; j++)
        {
            ...
            for (int k = 0; k < nf; k++)
            {
                ...
                H.block<8, 8>(jIdx, kIdx) += EF->adTarget[ijIdx] * accDM *
                                             EF->adTarget[ikIdx].transpose();
