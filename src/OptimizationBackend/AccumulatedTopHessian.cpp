                                                  ...
                                                  // 2=marginalize
{
    ...
    for (EFResidual* r : p->residualsAll)
    {
        ...
        if (mode == 1)
        {
            // compute Jp*delta
            __m128 Jp_delta_x =
                _mm_set1_ps(rJ->Jpdxi[0].dot(dp.head<6>()) +
                            rJ->Jpdc[0].dot(dc) + rJ->Jpdd[0] * dd);
            __m128 Jp_delta_y =
                _mm_set1_ps(rJ->Jpdxi[1].dot(dp.head<6>()) +
            ...
            for (int i = 0; i < patternNum; i += 4)
            {
                ...
                rtz        = _mm_add_ps(
                    rtz, _mm_mul_ps(_mm_load_ps(((float*)(rJ->JIdx)) + i),
                                    Jp_delta_x));
                rtz = _mm_add_ps(
                ...
                rtz = _mm_add_ps(
                    rtz, _mm_mul_ps(_mm_load_ps(((float*)(rJ->JabF + 1)) + i),
                                    delta_b));
    ...
    for (int h = 0; h < nframes[tid]; h++)
        for (int t = 0; t < nframes[tid]; t++)
        {
            ...
            H.block<8, 8>(hIdx, hIdx).noalias() +=
                EF->adHost[aidx] * accH.block<8, 8>(CPARS, CPARS) *
                EF->adHost[aidx].transpose();

            H.block<8, 8>(tIdx, tIdx).noalias() +=
                EF->adTarget[aidx] * accH.block<8, 8>(CPARS, CPARS) *
    ...
    int max, Vec10* stats, int tid)
{
    ...
    if (min == 0 && usePrior)
    {
        ...
        b[tid].head<CPARS>() +=
            EF->cPrior.cwiseProduct(EF->cDeltaF.cast<double>());
        for (int h = 0; h < nframes[tid]; h++)
        {
