                        ...
                        bool MT)
    {
        ...
        if (MT)
        {
            ...
            for (int i = 0; i < NUM_THREADS; i++)
            {
                ...
                Hs[i] =
                    MatXX::Zero(nframes[0] * 8 + CPARS, nframes[0] * 8 + CPARS);
                bs[i] = VecX::Zero(nframes[0] * 8 + CPARS);
            }

            red->reduce(
                boost::bind(&AccumulatedTopHessianSSE::stitchDoubleInternal,
        ...
        else
        {
            ...
            stitchDoubleInternal(&H, &b, EF, usePrior, 0,
                                 nframes[0] * nframes[0], 0, -1);
        }
...
private:
    void stitchDoubleInternal(MatXX* H, VecX* b,
                              EnergyFunctional const* const EF, bool usePrior,
