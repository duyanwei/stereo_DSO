...
private:
    ...
    void shiftUp(bool force)
    {
        if (numIn1 > 1000 || force)
        {
            _mm_store_ps(SSEData1k, _mm_add_ps(_mm_load_ps(SSEData),
                                               _mm_load_ps(SSEData1k)));
            numIn1k += numIn1;
            numIn1 = 0;
            memset(SSEData, 0, sizeof(float) * 4 * 1);
        }
        ...
        if (numIn1k > 1000 || force)
        {
            _mm_store_ps(SSEData1m, _mm_add_ps(_mm_load_ps(SSEData1k),
                                               _mm_load_ps(SSEData1m)));
            numIn1m += numIn1k;
            numIn1k = 0;
            memset(SSEData1k, 0, sizeof(float) * 4 * 1);
        }
...
public:
    ...
    /*
     * same as other method, just that x/y are composed of two parts, the first
     * 4 elements are in x4/y4, the last 6 in x6/y6.
     */
    ...
    inline void updateBotRight(const float a00, const float a01,
